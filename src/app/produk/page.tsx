...
import { Nut, Sprout, FlaskConical, Flame, Recycle, Store, Zap, Salad, ChefHatIcon } from "lucide-react";
      ...
      <div className="bg-white border-b border-[var(--color-cream-dark)]">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12">
          <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[var(--color-leaf)]/10 border border-[var(--color-leaf)]/20 mb-4">
            ...
            <span className="text-sm font-semibold text-[var(--color-leaf)]">Menu Kami</span>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 items-start">
              ...
              <div id="sauce-info-section">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {product.sauceVariants.map((sauce) => {
                    const SauceIcon = sauce.id === "pedas" ? Flame : Nut;
                    const iconColor = sauce.id === "pedas" ? "text-orange-500" : "text-[var(--color-turmeric)]";
                    const iconBg = sauce.id === "pedas" ? "bg-orange-50" : "bg-[var(--color-turmeric)]/10";
                    return (
                      <div
                        key={sauce.id}
                      ...
                      >
                        <div className={`w-9 h-9 rounded-xl ${iconBg} flex items-center justify-center flex-shrink-0`}>
                          <SauceIcon className={`w-5 h-5 ${iconColor}`} strokeWidth={1.75} />
                        </div>
                        <div>
          ...
          <div className="lg:col-span-2 lg:sticky lg:top-28">
            ...
            <div className="mt-4 grid grid-cols-3 gap-2 text-center">
              {[
                { icon: <Sprout className="w-4 h-4 text-emerald-600" strokeWidth={2} />, bg: "bg-emerald-50", text: "No Pengawet" },
                { icon: <Store className="w-4 h-4 text-[var(--color-turmeric)]" strokeWidth={2} />, bg: "bg-[var(--color-turmeric)]/10", text: "Segar Hari Ini" },
                { icon: <Zap className="w-4 h-4 text-[var(--color-leaf)]" strokeWidth={2} />, bg: "bg-[var(--color-leaf)]/10", text: "Order Cepat" },
              ].map((badge) => (
                <div
                  key={badge.text}
