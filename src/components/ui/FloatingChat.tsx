...
export default function FloatingChat() {
  ...
  const whatsappUrl = `https://wa.me/${SITE_CONFIG.whatsappNumber}?text=${encodeURIComponent(
    "Halo GoNuts Bites! Saya ingin bertanya mengenai produk kalian 😊"
  )}`;
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
      ...
      >
        <div className="glass-card rounded-2xl p-5 w-72 shadow-2xl">
          {/* Header */}
          <div className="flex items-center gap-3 mb-4">
            <div className="w-10 h-10 rounded-full gradient-leaf flex items-center justify-center shadow-sm flex-shrink-0">
              <span className="text-white font-black text-sm">G</span>
            </div>
            <div>
              <p className="font-bold text-sm text-[#1a1a1a]">GoNuts Bites CS</p>
              <div className="flex items-center gap-1.5">
                <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
                <span className="text-xs text-[#666]">Online sekarang</span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2 mb-4">
            <div className="bg-[var(--color-cream-dark)] rounded-2xl rounded-tl-sm px-4 py-3">
            ...
            <div className="bg-[var(--color-cream-dark)] rounded-2xl rounded-tl-sm px-4 py-3">
              <p className="text-sm text-[#1a1a1a]">
                Tanya soal menu, harga, atau cara pesan — kami siap membantu! 🥜
              </p>
            </div>
          </div>
